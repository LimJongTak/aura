...
export async function exportMileagePaymentExcel(semester: string, rows: MileagePaymentExportRow[]): Promise<void> {
  await writeExcel(
    ...
    "마일리지 장학금",
    ["학번", "이름", "학과", "승인 마일리지", "지급 금액", "지급 상태"],
    rows.map((r) => [r.studentId, r.studentName, r.department, r.approvedMileage, r.amount, r.paid ? "지급완료" : "미지급"]),
    [14, 10, 22, 12, 14, 10]
  );
}
...
export async function exportAdvancedPaymentExcel(semester: string, rows: AdvancedPaymentExportRow[]): Promise<void> {
  await writeExcel(
    ...
    "중고급 이수 장학금",
    ["학번", "이름", "등급", "지급 금액", "지급 상태"],
    rows.map((r) => [r.studentId, r.studentName, r.levels, r.amount, r.paid ? "지급완료" : "미지급"]),
    [14, 10, 14, 14, 10]
  );
}
